%% ======================================
% Script: Extract EEG segments, trials, and phases
%
% Purpose:
%   - Extract EEG segments based on task events (A_T[x], A→B, B→C, ..., F→mid(F,A_T[x+1])).
%   - Construct full EEG trials from start to mid(F, A_Tx).
%   - Organize EEG data into phases:
%       • rest_bef
%       • reaching
%       • grasping
%       • pulling
%       • rest_aft
%   - Visualize EEG phases consistent with Vissani 2021: Rest, Reaching, Grasping, Pulling.
%
% Special handling:
%   - Trials of same length enforced for subjects 'wue05', 'wue07', 'wue09', 'wue11'
%     using a window from 2.8 s before to 0.7 s after the F event.
%   - Excluded trials are handled explicitly per subject.
%   - Kinematic data is aligned to EEG trials.
%
% Outputs:
%   - EEG_segments: cell array [trial × region]
%   - EEG_phases: cell array [trial × phase]
%   - EEG_trials: concatenated trials [trial × time]
%   - kinematic_4EEG_trials: aligned kinematic velocity per trial
%   - baseline_EEG: baseline segment from start → A_T1
%   - trial durations (samples and ms)
%
% Author: Tommaso Marcantoni
% Date: 2026
%% ======================================

clear; clc;

subject_list = {'wue02', 'wue03', 'wue05', 'wue06', 'wue07', 'wue09', 'wue10', 'wue11'};
%subject_list = {'wue05'};

kin_subfolder = 'Preprocessed\EMG_KIN';

% Struct for baselines and length trials
all_baselines = struct();
all_durations = {};

[ALLEEG, EEG, CURRENTSET] = eeglab;
ALLEEG = [];


% === Trials rimossi nel preprocessing ===
removed_trials = struct( ...
    'wue03', [3, 4], ...   
    'wue05', [1, 4] ...   
);

for s = 1:length(subject_list)
    
    subject_id = subject_list{s};
    base_path  = 'C:\Users\tomma\OneDrive - University of Pisa\Desktop\TESI\Dataset_tesi';
    %preproc_path = fullfile(base_path, subject_id, 'Preprocessed_New', 'EEG');
    preproc_path = fullfile(base_path, subject_id, 'Preprocessed_01', 'EEG_manual');

    all_baselines.(subject_id) = {}; 

    original_EEG_path = fullfile(base_path, subject_id, 'Extracted', 'EEG_wEv');
    or_set_files = dir(fullfile(original_EEG_path, '*.set'));

    kin_path = fullfile(base_path, subject_id, kin_subfolder);
    
    set_files = dir(fullfile(preproc_path, '*.set'));
    kin_files = dir(fullfile(kin_path, '*_kinematic_block.mat'));
    
    for f = 1:length(set_files)

        set_file = set_files(f).name;
        fprintf('\nLoad the preprocessed EEG file: %s\n', set_file);
        
        EEG = pop_loadset('filename', set_file, 'filepath', preproc_path);
        EEG_or = pop_loadset('filename', or_set_files(f).name, 'filepath', original_EEG_path);

        Fs_EEG = EEG.srate; 

        event_types = {EEG.event.type};
        event_latencies = [EEG.event.latency];
        
        % Determine number of trials based on F_T events 
        is_FT = startsWith(event_types, 'F_T');                                      
        nums = cellfun(@(x) str2double(extractAfter(x, 'F_T')), event_types(is_FT)); 
        [num_trials, ~] = max(nums); % number of trials

        % === Gestione trial esclusi ===
        if isfield(removed_trials, subject_id) && numel(removed_trials.(subject_id)) >= 2 ...
                && removed_trials.(subject_id)(1) == f
            trial_to_remove = removed_trials.(subject_id)(2);
            fprintf('⚠️  %s file %d: escluso trial %d\n', subject_id, f, trial_to_remove);
        else
            trial_to_remove = [];
        end
        
        % === Lista dei trial validi ===
        valid_trials = setdiff(1:num_trials, trial_to_remove);
        num_valid_trials = numel(valid_trials);
        fprintf('→ Trials validi: %s\n', mat2str(valid_trials));
        
        % Nuove regioni (7 in totale)
        region_labels = {'start_A','A_B','B_C','C_D','D_E','E_F','F_midFA'};
        num_regions   = length(region_labels);
        
        % Prealloca cell array: trials × regions
        EEG_segments = cell(num_trials, num_regions);

        baseline_EEG = []; % Store baseline from beginning to A_T1
        
        %% === COSTRUZIONE SEGMENTI ===
        for new_t = 1:num_valid_trials

            t = valid_trials(new_t);  % trial originale

            % --- Trova evento A_Tx --> salto trial rimossi ---
            A_idx = find(strcmp(event_types, sprintf('A_T%d', t)));
            if isempty(A_idx)
                next_valid = find(~cellfun('isempty', regexp(event_types, 'A_T\d+')));
                next_valid_nums = cellfun(@(x) str2double(extractAfter(x, 'A_T')), event_types(next_valid));
                next_t = min(next_valid_nums(next_valid_nums > t));
                if isempty(next_t)
                    warning('Trial %d non trovato e nessun successivo valido.', t);
                    continue;
                else
                    fprintf('Trial %d mancante → salto al trial %d\n', t, next_t);
                    continue;
                end
            end

            % ---- 1. start → A_T1 ----
            A1_idx  = find(strcmp(event_types, sprintf('A_T%d', t)));
            if isempty(A1_idx)
                warning('Trial %d: A_T%d not found.', t, t);
                continue;
            end

            end_latency   = event_latencies(A1_idx);

            if t == 1 
                start_latency = 1; % inizio recording

                % Define baseline
                baseline_EEG = EEG.data(:, start_latency:end_latency);

                % Save in struct
                all_baselines.(subject_id){f} = baseline_EEG;
            else 
                start_latency = mid_FA + 1;
            end

            EEG_segments{new_t,1} = EEG.data(:, start_latency:end_latency);

            % ---- 2-6. regioni canoniche A→B ... E→F ----
            base_regions = {'A','B','C','D','E','F'};
            for r = 1:(length(base_regions)-1)
                start_label = sprintf('%s_T%d', base_regions{r}, t);
                end_label   = sprintf('%s_T%d', base_regions{r+1}, t);
                
                start_idx = find(strcmp(event_types, start_label));
                end_idx   = find(strcmp(event_types, end_label));
                if isempty(start_idx) || isempty(end_idx)
                    warning('Trial %d: %s→%s not found.', t, base_regions{r}, base_regions{r+1});
                    continue;
                end
                EEG_segments{new_t,r+1} = EEG.data(:, event_latencies(start_idx)+1:event_latencies(end_idx));
            end
            
            % ---- 7. F → mid(F, A_T1_next) ----
            F_idx = find(strcmp(event_types, sprintf('F_T%d', t)));

            if ~isempty(F_idx)
                F_lat = event_latencies(F_idx);
            
                % Trova A del trial successivo (se esiste)
                nextA_idx = find(strcmp(event_types, sprintf('A_T%d', t+1)));
            
                if isempty(nextA_idx)
                    % Se il trial successivo è stato rimosso → cerca un "boundary" dopo F
                    boundary_idx = find(strcmp(event_types, 'boundary'));
                    boundary_lat = event_latencies(boundary_idx);
                    boundary_after_F = boundary_lat(boundary_lat > F_lat);
            
                    if ~isempty(boundary_after_F)
                        end_lat = boundary_after_F(1);
                    
                        % Verifico se questo boundary è l'ultimo evento del dataset
                        is_last_boundary = end_lat == EEG.event(end).latency;
                    
                        if is_last_boundary
                            % Ultimo trial → chiudo regolarmente al boundary finale
                            mid_FA = end_lat;
                            fprintf('✅ Trial %d: ultimo trial, fine al boundary finale (%.1f s)\n', ...
                                    t, end_lat / Fs_EEG);
                        else
                            % Trial intermedio → A_T mancante, uso boundary come sostituto
                            mid_FA = end_lat;
                            fprintf('⚠️ Trial %d: A_T%d mancante, uso boundary a %.1f s come fine rest_aft\n', ...
                                    t, t+1, end_lat / Fs_EEG);
                        end
                    else
                        % Nessun boundary → fino alla fine del segnale
                        end_lat = EEG.pnts;
                        fprintf('⚠️ Trial %d: A_T%d e boundary mancanti → fino a fine recording\n', ...
                                t, t+1);
                    end
                else
                    % Se A_next esiste → prendo la metà tra F e A_next
                    A_next_lat = event_latencies(nextA_idx);
                    end_lat = round(F_lat + (A_next_lat - F_lat)/2);
                    mid_FA = end_lat;
                end
            
                EEG_segments{new_t,7} = EEG.data(:, F_lat+1:end_lat);
            
            else
                warning('Trial %d: F_T%d non trovato.', t, t);
            end
        end

        fprintf('✅ EEG segments creati: %d trials × %d regions\n', size(EEG_segments,1), size(EEG_segments,2));

        % === Store structure containing data divided by phases rather than
        % events regions
        phase_names   = {'rest_bef','reaching','grasping','pulling', 'rest_aft'};
        phase_regions = { 1, [2 3], 4, [5 6], 7 };  

        % Preallocate cell array: trials × phases
        EEG_phases = cell(num_valid_trials, length(phase_names));

        % Preallocate cell array for trial division
        EEG_trials = cell(num_valid_trials, 1);

        % matrix for trial durations
        trial_durations = zeros(num_valid_trials, 2);

        for new_t = 1:num_valid_trials
            t = valid_trials(new_t);
            A1_idx  = find(strcmp(event_types, sprintf('A_T%d', t)));
            A1_latency = event_latencies(A1_idx);
            trial_data = [];
            for p = 1:length(phase_names)
                tmp_data = [];
                for r = phase_regions{p}
                    if new_t == 1 && r == 1
                        start_rest_bef = A1_latency - 250;
                        tmp_data = [tmp_data, EEG_segments{new_t,r}(:, start_rest_bef:end)]; % concat along time
                    elseif r <= size(EEG_segments,2) && ~isempty(EEG_segments{new_t,r})
                        tmp_data = [tmp_data, EEG_segments{new_t,r}]; % concat along time
                    end
                end
                EEG_phases{new_t,p} = tmp_data;
                trial_data = [trial_data, tmp_data];  
            end
            EEG_trials{new_t,1} = trial_data;

            dur_samples = length(trial_data);
            dur_ms      = (dur_samples / Fs_EEG) * 1000;
            
            trial_durations(new_t,1) = dur_samples;
            trial_durations(new_t,2) = dur_ms;

            all_durations(end+1,:) = {subject_id, f, new_t, ...
                                      trial_durations(new_t,1), ...
                                      trial_durations(new_t,2)};
        end

         %% === Carica kinematica ===
        kin_file = kin_files(f).name;
        
        if exist(fullfile(kin_path, kin_file), 'file')
            load(fullfile(kin_path, kin_file), 'kinematic_block'); % Load smooth velocity for current block
        else
            warning('Nessun file cinematico trovato per %s - %s', subject_id, set_file);
            continue;
        end

        % Isolate region of interest KIN signal
        event_types_or = {EEG_or.event.type};
        lat_TEN_idx_all = find(strcmp(event_types_or, 'TENStrigger'));
        lat_TEN_idx = lat_TEN_idx_all(1);
        lat_TEN = EEG_or.event(lat_TEN_idx).latency;

        A1_idx = find(strcmp({EEG_or.event.type}, 'A_T1'));
        F_idx  = find(strcmp({EEG_or.event.type}, sprintf('F_T%d', num_trials)));

        A1_idx_prep = find(strcmp({EEG.event.type}, 'A_T1'));
        A1_latency_prep = event_latencies(A1_idx_prep);
        
        first_latency = lat_TEN;
        second_latency = EEG_or.event(F_idx).latency;
        
        % Converti gli indici EEG in campioni KIN 
        start_idx_kin = (first_latency + 50);
        end_idx_kin   = (second_latency+300);
        
        vel_block = kinematic_block.velocity(start_idx_kin:end_idx_kin);
        
        time = (0:length(vel_block)-1) / Fs_EEG;

        if (strcmp(subject_id, 'wue03') && f == 3)
            trial_to_remove = 4;
        elseif (strcmp(subject_id, 'wue05') && f == 1)
            trial_to_remove = 4;
        else
            trial_to_remove = [];
        end

        event_latencies_or = round([EEG_or.event.latency]);
        
        if ~isempty(trial_to_remove)
            % Trova eventi delimitanti la finestra da saltare
            F_idx_pre  = find(strcmp(event_types_or, sprintf('F_T%d', trial_to_remove - 1)));
            A_idx_curr = find(strcmp(event_types_or, sprintf('A_T%d', trial_to_remove)));
            F_idx_curr = find(strcmp(event_types_or, sprintf('F_T%d', trial_to_remove)));
            A_idx_next = find(strcmp(event_types_or, sprintf('A_T%d', trial_to_remove + 1)));
        
            F_lat_pre  = event_latencies_or(F_idx_pre);
            A_lat_curr = event_latencies_or(A_idx_curr);
            F_lat_curr = event_latencies_or(F_idx_curr);
            A_lat_next = event_latencies_or(A_idx_next);
        
            mid_FA_pre  = round(F_lat_pre + (A_lat_curr - F_lat_pre)/2);
            mid_FA_post = round(F_lat_curr + (A_lat_next - F_lat_curr)/2);
        
            % Converti in indici cinematici (stesso offset del block)
            kin_start = mid_FA_pre - first_latency + 50;   % prima di mid_FA
            kin_end   = mid_FA_post - first_latency + 50;  % fino a mid_FA post
        
            vel_block(kin_start:kin_end) = [];
        end

        % Estrarre trial validi
        kinematic_4EEG_trials = cell(num_valid_trials, 1);

        offset = A1_latency_prep - 250;

        % Indice iniziale nella velocità totale
        start_idx = offset;

         for new_t = 1:num_valid_trials
            t = valid_trials(new_t);
            trial_len = size(EEG_trials{new_t}, 2);
        
            end_idx = start_idx + trial_len - 1;
        
            if end_idx <= length(vel_block)
                trial_vel = vel_block(start_idx:end_idx);
                kinematic_4EEG_trials{new_t} = trial_vel;
            else
                warning('⚠️ Trial %d (%s): cinematico troppo corto (end_idx=%d, vel_len=%d)', ...
                    t, subject_id, end_idx, length(vel_block));
                kinematic_4EEG_trials{new_t} = vel_block(start_idx:end);
            end
        
            start_idx = end_idx + 1;
        end
        %% === SAVE ===
        save(fullfile(preproc_path, [set_file(1:end-4) '_EEG_trialsByRegionAndPhase.mat']), ...
            'EEG_trials', 'EEG_segments', 'EEG_phases', 'region_labels', 'phase_names', 'phase_regions','kinematic_4EEG_trials', 'baseline_EEG');
    end
end

